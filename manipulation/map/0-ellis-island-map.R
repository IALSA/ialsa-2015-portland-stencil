...
# ---- declare-globals ----------------------------------------------
# data_path_input  <- "./data/unshared/raw/dto-map.rds"
data_path_input  <- "../MAP/data-unshared/derived/ds0.rds"
metadata_path_input <- "./data/shared/meta/map/meta-data-map.csv" # input file with your manual classification
...
meta_data <- dto[["metaData"]] %>%
  ...
  # dplyr::select(name, varname, type, label, construct) %>%
...
d_rules <- dto[["metaData"]] %>%
  ...
  dplyr::select(name, varname ) # leave only collumn, which values you wish to append
names(ds) <- d_rules[,"varname"]
...
saveRDS(dto, file="./data/unshared/raw/map/dto-map.rds", compress="xz")


# ---- object-verification ------------------------------------------------
# the production of the dto object is now complete
# we verify its structure and content:
dto <- readRDS("./data/unshared/raw/map/dto-map.rds")
