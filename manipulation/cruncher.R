...
ds_catalog <- ds_catalog %>%
  ...
  dplyr::mutate(
    file_exists         = file.exists(path_inp),
    directory           = dirname(path_inp),
    file_name_regex     = gsub("\\.", "\\\\\\\\.", basename(path_inp)) #Escape all dots, for the sake of the regex.
  )

# ---- identify-missing-inputs -----------------------------------------------------------
if( any(!ds_catalog$file_exists) ) {
  missing_files <- paste(ds_catalog$path_inp[!ds_catalog$file_exists], collapse="\n")
  cat(missing_files)
  warning("The previous ", scales::comma(sum(!ds_catalog$file_exists)), " input files are missing and will not be run.")
}
